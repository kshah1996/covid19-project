...
`mwgrw` computes the estimates of a Poisson generalized linear mixed model with a log link for COVID-19 data from the CSSE at Johns Hopkins. Other data included comes from the World Bank and the Global Health Secuiry Index. The response is the number of new cases, while the predictors include an intercept, time, time$^2$, GHS score, percentage of population over age 65, and percentage of population living in urban areas. Random effects can be adjusted but is defaulted to intercept and time (Helper functions provided as well).
...
`getRF` returns a graph of the random forest model and predictions specified by the user.
...
We see extremely large MSE values, especially for the US. This is somewhat surprising since the model seems to fit fairly well. However, it is important to note that since we are working on the scale of total cases, counts are very large and errors in under or over prediction compiles over time. This can lead to very large MSEs calculated for future predictions.
