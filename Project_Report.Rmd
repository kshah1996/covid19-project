...
Our initial dataset comes from the Center for Systems Science and Engineering (CSSE) at Johns Hopkins University, a team which has been refreshing the dataset daily during the ongoing COVID-19 crisis. The dataset includes Coronavirus total cases, recoveries, active cases, and deaths per day by country, with information for certain regions given in more detail (e.g. US states). 

For this project, the focus was confirmed cases. Dates from the original dataset were standardized by relabeling the date at which a country passes 50 total cases as "Day 0", with every day past that used for modeling purposes. A variable for new cases was created by lagging over total cases. 
...
The Global Health Security (GHS) Index is a comprehensive assessment of a country's health security capabilities. Countries receive an overall score as well as individual scores for each of the 6 categories:
...
The Hopkins, World Bank, and GHS Index data were combined so that, along with each country's case counts, we could use GHS Score, urban population percentage, and percentage of population older than 65. For our project, the aforementioend variables were used along with country, day and number of new cases. 
...
# Packaging
All functions needed to model and predict number of new cases of COVID-19 can be found in our R package: `covid`. Below is a description of each of these functions along with their code. All functions have tests written for them and the packages pass these tests using `test_package("covid")` from the `testthat` package. Methodology behind the creation of these functions will be explained in detail in following sections.

## *Metropolis Within Gibbs Random Walk*

`mwgrw` computes the estimates of a Poisson generalized linear mixed model with a log link for COVID-19 data from the CSSE at Johns Hopkins. Other data included comes from the World Bank and the Global Health Security Index. The response is the number of new cases, while the predictors include terms for intercept, time, time$^2$, GHS score, percentage of population over age 65, and percentage of population living in urban areas. Random effects can be adjusted but is defaulted to intercept and time (Helper functions provided as well).

Function Inputs:

* `beta`: An intial vector of length 6 to be the starting values of beta for the MCEM algorithm. Default values avaliable as the output from glmer function.

* `Sigma_gamma`: An intial matrix of size qxq, where q is the dimension of the random effects. Default values are 2x2. Default random effects would require a 2x2 matrix.

* `M`: Chain length for each country in the MCEM algorithmn. Defaults to 1000.

* `Z `:A vector that contains the subset of columns of X to be random effects. Defaults to intercept and time only.

* `control`: A list of control parameters for the MCEM algorithm. tol is the convergence tolerance, and maxit is the maximum number of iterations

* `verbose`: A Boolean indicating if output should be printed at each iteration
...
`mwgrwMSE` calculates the MSE for a specific country or every country based on their predictions for the 8 days following April 03, 2020.

Function Inputs:

* `country_name`: A character input corresponding to the country, if left empty the function will calculate the MSE for all countries together

* `glmer_MSE`: A Boolean determining if the MSE should be calculated for the MCEM GLMM or the glmer function
...
`countrygraph` creates a graph for user specified country that displays (in black) observed number of new cases of COVID-19 from their baseline day of 50 cases confirmed until April 3, 2020. The Poisson Generalized Linear Mixed Model is graphed over the same period (in red) to show accuracy of model in explaining number of new cases for that country. By specifying glmer_results=TRUE, the glmer results are also graphed over the same period (in blue). An option to generate the predictions for the next 8 days is present as well. If set to true, predictions will be displayed as a dotted line extension of the GLMM (red) and glmer (blue) curves.
...
`getRF` returns a graph of the random forest model and predictions for the number of days specified by the user.

Function Inputs:
  
* `Country_Name`: a character value written exactly as the country is listed in the dataset  
* `Pred_Day`: The number of days the user wants to predict in the future
...
getRF <- function(Country_Name, Pred_Day = 8) {
  ...
  # Obtain MSE for next 8 days of data
  prediction_df$se <- (prediction_df$new_cases - prediction_df$pred)^2
  MSE <- sum(prediction_df$se[1:nrow(dat2_rf_preds_country)])
...
`forecast` returns a list including a graph of the model and forecast as well as a data frame that includes all previous data and forecasted predictions for the length of `numPred`. 

Function Inputs:
  
* `country`: a character value written exactly as the country is listed in the dataset  
* `data`: Any data frame that includes country name, total cases, and the country's total population    
...
### Model Creation
We let $y_{ij}$ be the number of new cases country $i$ has on day $j$ and assumed each $y_{ij} \sim Poisson(\lambda_{ij})$, where the mean and variance of $y_{ij}$ is assumed to be some $\lambda_{ij}$.

Our model includes an intercept, covariates for time and time squared, and covariates for Global Health Security index, proportion of the population 65 or older, and proportion of population living in an urban setting. Thus, the covariate vector pertaining to the $i^{th}$ country in the $j^{th}$ month is defined as $x_{ij} = (1, t_{ij}, t^2_{ij}, GHS, AgeGEQ65, UrbanPop)$.

Since we assume an intercept, $\beta_0$, in our model, and thus we have $\beta = (\beta_0, \beta_1, \beta_2, \beta_3, \beta_4, \beta_5, \beta_6)^T$.

With our assumption that the counts can be modeled with Poisson GLMM, we have $log(\lambda_{ij})=x_{ij}\beta+\gamma_{i1}+\gamma_{i2}t_{ij} = x_{ij}\beta+z_{ij}\gamma_i$, where $\gamma_i$ is a vector of unobserved country-level random effects assumed to follow a bivariate normal distribution with mean 0 and covariance matrix G.
...
Thus, we must perform separate integrals for each $\left(\begin{array}{cc}\gamma_{i1} \\\gamma_{i2}\end{array}\right)$, which share the covariance matrix determined by $G=\left(\begin{array}{cc}g_{11} & g_{12} \\g_{21} & g_{22}\end{array}\right)$.

Letting $n$ be the total number of countries and $n_i$ be the observed days of recorded cases in each country, we write the likelihood:
...
and taking the log, we can write the log-likelihood:
...
where $\lambda_{ij}^{(t)}=e^{x_{ij}\beta^{(t)}+z_{ij}\gamma_{i}}$. If we could sample from the posterior distribution of $\gamma_i$ we could approximate this integral through a MC approach.
...
Prophet is based on a decomposable time series model as follows: 
...
While the method allows for seasonal and holiday effects, we did not include these in our model as we do not believe they apply to COVID-19 cases. 
...
Running the model we have on the dataset lead to parameter estimates as follows *(code not explicitly ran here, it was ran on the cluster)*.
...
We can compare these to estimates from the glmer package in R.
...
The glmer model is generally a bit higher than our model but the fits are comparable. It seems that, for the US, the fit is acceptable for the 8 days predicted. For China and South Korea, both countries have almost finished their curve, which leads to predictions being fairly accurate as they are near zero. We address the anomaly seen in the China graph on Feb 06, 2020 later. Aside from this specific date, our predictions for China are fairly accurate. The fits for South Korea and Italy seem off. This is due to the non-bell-shaped nature of their curves and the sporadic changes in new case number. These break our assumptions for our model which cause the fits to be questionable. Overall, comparing the MSEs for predictions over all countries for both our model and the glmer model, our model has a MSE of about half that of glmer. Although, the outlier points and countries with bad fits can effect this number drastically.

#### MSE Calculation

MCEM GLMM MSE for prediction for the next 8 days (April 4, 2020 through April 13, 2020) in all countries:

```{r warning=FALSE}
mwgrwMSE(,FALSE)
```

`glmer` MSE for prediction in all countries:

```{r warning=FALSE}
mwgrwMSE(,TRUE)
```

MSEs for prediction in a select few countries:

Country   | Poisson GLMM | GLMER    
--------- | ------------ | -------- 
US        |  6410718     | 90351421 
S. Korea  |  3047.854    | 3002.723 
Italy     |  6379592     | 4284941  
China     |  4216.611    | 4216.598 

## *Machine Learning*

### *Random Forest*

After running the random forest model, we observed that fits to the data and predictions were decent for some countries, and entirely inaccurate for others. The fit was dependent on how close the country's covariates were to the other countries with similar trajectories. For outlier countries like the United States, for example, fitting was entirely inaccurate, let alone predictions. For other countries, such as Italy (or China and South Korea, which have finished their curves), fitting was decent. In both cases, however, random forest clearly cannot extrapolate, as predictions past a certain date are simply a flat line, which will not be observed in reality. Random Forest therefore cannot be used for future prediction of time points outside the given data.
...
For example, for the United States, as plotted above, the random forest model highly overpredicts early on and underpredicts as the situation worsens, whereas the GLMM fit for the US was quite accurate (even with the US being an outlier in terms of number of daily new cases) because it was prespecified to be a bell curve. On the other hand, random forest struggles because its estimates for the United States are biased towards the outcome of all countries with a similar `GHS_Score`, `AgeGEQ65`, and `UrbanPop`. 

#### MSE Calculation

We now look at the MSE's for the random forest prediction compared to the next eight days of each country's data. 

Country   |  Random Forest 
--------- |  ------------------------
US        | 354404787
S. Korea  | 4729.271
Italy     | 514311.1
...
Clearly, the Poisson GLMM works much better for the US, South Korea, and China. For Italy, the random forest MSE is lower that the Poisson GLMM MSE, but keep in mind that random forest makes the same, flat prediction for Italy for every day of the future, as shown in the graphs above. Therefore, although the RF model happened to have decent predictions for Italy for the next few days, predictions would not hold up for dates further in the future.

Also keep in mind that each country's MSE is not comparable with any other country's MSE, because the scale of the number of new cases is vastly different in each case.
...
We see, in general, the model fits fairly well if the country has not reached its inflection point. You can see this from the graphs for the US and Italy. However, if the country has been effective at flattening the curve, such as China and South Korea, the method can not properly capture the inflection point nor make accurate forecasts for future cases. Additionally, the exponential growth fit cannot be used for future predictions past a certain time point because each country's curve is bound to flatten eventually.

#### MSE Calculation

MSEs were calculated on a country-by-country basis (for the next 8 days of prediction) with Prophet. Please note, the MSEs listed in the table below are for total number of cases and thus cannot be compared with the MSEs from the two methods reported previously. 
...
Country   |  Prophet 
--------- |  ------------------------
US        | `r format(US_mse_prophet,scientific=F)`
S. Korea  |  `r format(SKorea_mse_prophet, scientific = F)`
Italy     |`r format(Italy_mse_prophet, scientific = F)`
China     | `r format(China_mse_prophet, scientific = F)`

We see extremely large MSE values, especially for the US. This is somewhat surprising since the model seems to fit fairly well. However, it is important to note that since we are working on the scale of total cases, counts are very large and errors in under or over prediction compile over time. This can lead to very large MSEs calculated for future predictions.
...
In this setting, a random forest modeling approach and the majority of available machine learning and time-series modeling approaches are limited by their inability to handle correlated, longitudinal data.

### Random Forest vs. Poisson GLMM

One key limitation of random forest for this dataset is the fact that countries with similar GHS scores and population structures still had vastly different disease curves and trajectories with respect to new COVID cases per day. It is nearly impossible to gather a collection of covariates that accounts for the differences in curve structure between every country. Whether such a set of covariates even exists is unlikely, let alone actually finding it. Therefore, without being able to predict for an individual country based off of its previous time points (as was the case with the longitudinal Poisson GLMM modeling approach), making country-specific predictions through a random forest model was unlikely to be fruitful.
...
### Prophet vs. Poisson GLMM

On the other hand, our time-series modeling approach, Prophet, suffered from the opposite problem of random forest. Unlike random forest, which struggled to incorporate a country's previous values into the model, Prophet could *only* use a country's previous time points to predict future time points and was unable to incorporate information from other countries' pandemic curves. Therefore, Prophet could only be used reliably on countries with a large amount of time points, making global predictions impossible.

### Heuristic Argument for the GLMM Approach
...
De Angelis et. al. (2015) describe how Markov chain Monte Carlo (MCMC) approaches have become a 'gold standard' in the world of modeling infectious diseases. However, they note several complications that we can fully understand after working through this project. These include: writing a likelihood in closed form, data augmentation that involves imputation of more unknowns than feasible to handle, and difficulty implementing the model in real time due to high level of computational effort.

These computational limitations can be seen when running our Poisson GLMM code. We ran the modeling on longleaf and it took 3 hours to complete. With real-time data it is not out of the ordinary to run models at least once a day if not more frequently depending on the reliability of the data coming in. When one is attempting to capture the true nature of the outbreak in real time it is critical to have models which incorporate accurate, up-to-date information and receive results with a turnaround time that is still applicable in the situation. 
