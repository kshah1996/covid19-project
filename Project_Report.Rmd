...
Below is a preview of what our final data set looks like. Each country has a row for each day of recorded cases from baseline (day of 50 cases) until April 11, 2020.
...
where $\Gamma_{ik}$ is one of the $M$ samples from the $i$-th country, and $\lambda_{ijk}^{(t)}=e^{x_{ij}\beta^{(t)}+z_{ij}\Gamma_{ik}}$. To sample from this multivariate posterior, we use an adaptive Metropolis-within-Gibbs random walk.
...
Additionally, we decided to consider a time-series modeling method called Prophet, originally developed by Facebook Research. **[feel free to add a brief description here]**
...
dat2_rf <- datDisplay %>% select(Country.Region, day, GHS_Score, AgeGEQ65, UrbanPop, new_cases) %>%
  drop_na()
...
ggplot(dat2_rf_preds_usa, aes(x = day)) +
