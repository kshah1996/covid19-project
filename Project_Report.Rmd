...
Below is the predictions of our GLMM model for four countries, the United States, China, South Korea, and Italy. The model was built on data up until April, 03 202, and then we use the predictions to project the next 8 days. Since the original data is from the past, we also know the true observed number of new cases in these countries as well. The dashed lines indicated time points that were not included in the generation of our model.
...
```{r}
US <- countrygraph("US", prediction = TRUE, Pred_Day = 8)
China <- countrygraph("China", prediction = TRUE, Pred_Day = 8)
SK <- countrygraph("Korea, South", prediction = TRUE, Pred_Day = 8)
Italy <- countrygraph("Italy", prediction = TRUE, Pred_Day = 8)
...
forecast <- function(country, data, numPred){
  ...
  print(plot(now, forecast, plot_cap=F, uncertainty = T, ylabel = paste(country, "total cases"), xlabel="Date"))
...
US <- forecast("US", dat_limit, 10)
SouthKorea <- forecast("Korea, South", dat_limit, 10)
Italy <- forecast("Italy", dat_limit, 10)
China <- forecast("China", dat_limit, 10)
```
...
(Emily - I had written this paragraph before when we had just a general limitations section - feel free to use or delete!)
