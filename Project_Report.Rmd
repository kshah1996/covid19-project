...
Our model includes an intercept, covariates for time and time squared, and covariates for Global Health Security index, proportion of the population 65 or older, and proportion of population living in an urban setting. Thus, the covariate vector pertaining to the $i^{th}$ countrh in the $j^{th}$ month is defined as $x_{ij} = (1, t_{ij}, t^2_{ij}, GHS, AgeGEQ65, UrbanPop)$.
...
The glmer model is generally a bit higher than our model but the fits are comparable. It seems that for the US the fit is acceptable for the 8 days. For China and South Korea, both countries have almost finished their curve, which leads to predictions being fairly accurate as they are near zero. We address the anomaly seen in the China graph on Feb 06, 2020 later. Aside from this specific date, the fits for South Korea and Italy seem off. This is due to the non-bell shaped nature of their curves and the sporadic changes in new case number. These break our assumptions for our model which causes the fits to be questionable. Overall, comparing the MSEs for predictions over all countries for both our model and the glmer model, our model has a MSE of about half that of glmer. Although, the outlier points and countries with bad fits can effect this number drastically.

MCEM GLMM MSE for prediction for the next 8 days in all countries:
...
Clearly, the Poisson GLMM works much better for the US and South Korea, and China. For Italy, the random forest MSE is lower, but keep in mind that random forest makes the same, flat prediction for Italy for every day of the future, as shown in the grahps above. Therefore, although the RF model happened to have decent predictions for Italy for the next few days, predictions would not hold up for dates further out into the future.
